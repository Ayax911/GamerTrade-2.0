...
namespace APiGamer.Servicios
{
    public class ServicioConsultas : IServicioConsultas
    {
        ...
        public async Task<(bool Esvalida, string mensaje)> ValidarConsulta(string consulta, Dictionary<string, object>? parametros, string[] tablasProhibidas)
        {
            ...
            // ✅ Permitir SELECT, WITH (CTE), y EXEC (Stored Procedures)
            string consultaTrimUpper = consulta.ToUpper().Trim();
            if (!consultaTrimUpper.StartsWith("SELECT") && 
                !consultaTrimUpper.StartsWith("WITH") && 
                !consultaTrimUpper.StartsWith("EXEC"))
            {
                return await Task.FromResult((false, "La consulta debe ser SELECT, WITH (CTE), o EXEC (Stored Procedure)."));
            }
            return await Task.FromResult((true, "Consulta válida"));
        }
        /// <summary>
